...
class DjazztroBackend(DjangoTemplates):
    ...
    def get_template_dev(self, template_name, dev_port):
        try:
            ...
            template_text = resp.text.replace("src=\"/", f"src=\"http://localhost:{dev_port}/")
            return self.from_string(template_text)
        except requests.ConnectionError:
            raise TemplateFetchError(f"Can't connect to Astro, is it running? (Tried: http://localhost:{dev_port})")
